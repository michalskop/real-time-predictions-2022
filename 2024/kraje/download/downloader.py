...
path = "/home/michal/dev/real-time-predictions-2022/2024/kraje/download/" # ** for testing only **
...
if settings['test']:
  teststr = '-test'
  url = settings['url_server_test']
  url0 = url + "vysledky_okrsky.xml"  # testing on local data
else:
  teststr = ''
  url = settings['url_server']
  url0 = url + "vysledky_okrsky.xml"
...
while (not ok) or (m > 10):
  try:
    r = requests.get(url0, verify=None)
...
for n in batches_to_download:
  ...
  while (not ok) or (m > 10):
    try:
      r = requests.get(url + 'vysledky_okrsky_' + str(n).zfill(5) + ".xml", verify=None)
      obj = xmltodict.parse(r.content)
      ok = True
    except:
      m += 1
  if ok:
    ...
    # save the batch
    fpath = path + 'batches' + teststr + '/vysledky_okrsky_' + str(n).zfill(5) + '.xml'
